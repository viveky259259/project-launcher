...
async fn bootstrap_super_admin(db: &Db) {
    ...
    // Calculate key_hash and key_prefix
    use sha2::{Sha256, Digest};
    let mut hasher = Sha256::new();
    hasher.update(&bootstrap_key);
    let key_hash = format!("{:x}", hasher.finalize());
    let key_prefix = bootstrap_key.chars().take(12).collect::<String>();
...
async fn main() -> anyhow::Result<()> {
    ...
    let mongodb_uri = std::env::var("MONGODB_URI").unwrap_or_else(|_| {
        tracing::warn!("MONGODB_URI not set — falling back to local MongoDB at localhost:27017");
